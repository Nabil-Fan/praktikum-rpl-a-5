{{--
    ...
    Data dari: App\Http\Controllers\Admin\UserController@index
    Variabel: $users (Paginator), $search, $filter, $counts
--}}
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title>Pengguna — EcoEats Admin</title>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .ftab { padding:.35rem .9rem; border-radius:999px; font-size:.78rem; font-weight:600; border:1.5px solid rgba(56,44,35,.12); color:rgba(56,44,35,.5); text-decoration:none; transition:all .15s; display:inline-flex; align-items:center; gap:.4rem; }
        .ftab:hover { border-color:var(--laurel); color:var(--olive); }
        .ftab.active { background:var(--olive); color:white; border-color:var(--olive); }
        .ftab-count { font-size:.68rem; padding:.05rem .45rem; border-radius:999px; }
        ...
        .u-avatar { width:32px; height:32px; border-radius:8px; background:var(--laurel); color:white; display:inline-flex; align-items:center; justify-content:center; font-weight:700; font-size:.8rem; margin-right:.6rem; flex-shrink:0; }
        .u-name-cell { display:flex; align-items:center; }
        .u-sub { font-size:.73rem; color:var(--camel); margin-top:.1rem; }

        .badge-active   { display:inline-block; padding:.18rem .6rem; border-radius:999px; font-size:.67rem; font-weight:700; background:rgba(95,111,82,.12); color:var(--olive); }
        .badge-inactive { display:inline-block; padding:.18rem .6rem; border-radius:999px; font-size:.67rem; font-weight:700; background:rgba(180,60,60,.08); color:#a03030; }
        ...
        .btn-restore { background:var(--olive); color:white; border-color:var(--olive); }
        .btn-restore:hover { background:var(--charcoal); border-color:var(--charcoal); }
...
<div class="layout">
    ...
    <main class="content">
        ...
        <div class="pg-head" style="display:flex;align-items:flex-start;justify-content:space-between;gap:1rem">
            <div>
                <h1>Manajemen Pengguna</h1>
                <div class="pg-crumb">EcoEats › Admin › Pengguna</div>
            </div>
            <a href="{{ route('admin.accounts.create') }}"
               style="display:inline-flex;align-items:center;gap:.4rem;padding:.55rem 1.1rem;background:var(--olive);color:white;border-radius:9px;font-family:var(--font-b);font-size:.8rem;font-weight:700;text-decoration:none;white-space:nowrap;transition:background .2s"
               onmouseover="this.style.background='var(--charcoal)'" onmouseout="this.style.background='var(--olive)'">
                ＋ Tambah Akun
            </a>
        </div>
        ...
        <div class="toolbar">
            <form method="GET" action="{{ route('admin.users.index') }}" class="search-wrap">
                ...
                <input class="search-input" type="text" name="search"
                       value="{{ $search }}" placeholder="Cari nama atau email…">
                <button type="submit" class="search-btn">Cari</button>
            </form>
            <div class="filter-tabs">
                <a href="{{ route('admin.users.index', ['filter'=>'active', 'search'=>$search]) }}"
                   class="ftab {{ $filter==='active' ? 'active' : '' }}">
                   Aktif <span class="ftab-count">{{ $counts['active'] }}</span>
                </a>
                <a href="{{ route('admin.users.index', ['filter'=>'deleted', 'search'=>$search]) }}"
                   class="ftab {{ $filter==='deleted' ? 'active' : '' }}">
                   Nonaktif <span class="ftab-count">{{ $counts['deleted'] }}</span>
                </a>
                <a href="{{ route('admin.users.index', ['filter'=>'all', 'search'=>$search]) }}"
                    ...
                    <tr>
                        ...
                        <td>
                            <form method="POST" action="{{ route('admin.users.toggle', $u) }}" style="display:inline"
                                  onsubmit="return confirm('{{ $u->deleted_at ? 'Aktifkan kembali' : 'Nonaktifkan' }} akun {{ addslashes($u->name) }}?')">
                                @csrf
                                <button type="submit" class="btn-sm {{ $u->deleted_at ? 'btn-restore' : 'btn-del' }}">
                                    {{ $u->deleted_at ? 'Aktifkan' : 'Nonaktifkan' }}
                                </button>
                            </form>
                        </td>
                    </tr>
